...
class FetchRowCSV:
    ...
    def __init__(self):
        ...
        df = pd.read_csv("trail.csv", dtype=np.float)

        for index, rows in df.iterrows():
            # Create list for the current row
            record_list = [float(rows.MSSubClass), float(rows.LotArea), float(rows.YearBuilt), float(rows.BedroomAbvGr),
                           float(rows.TotRmsAbvGrd)]
